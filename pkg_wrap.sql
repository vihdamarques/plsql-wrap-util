...
function fnc_unwrap(p_name  in varchar2
               	,p_owner in varchar2 default null) return clob as
  ...
  cursor obj is
	select object_name name
 	...
 	from all_objects
 	where object_name  = upper(p_name)
   	and owner    	= nvl(upper(p_owner), user)
   	and object_name != 'PKG_WRAP'
   	and object_type in ('PROCEDURE','PACKAGE BODY','FUNCTION')
 	order by object_type;

  cursor src(pp_name in varchar2, pp_type in varchar2, pp_owner in varchar2) is
	select case when (line = 1) then substr( text, instr( text, chr( 10 ), 1, 20 ) + 1 ) else text end text
  	from all_source
  	where name  = upper(pp_name)
    	and type  = upper(pp_type)
